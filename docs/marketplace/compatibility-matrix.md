...
| Active Write/Edit guard | Node.js 20+ and Bash 3.2+ | `node scripts/validate-runtime-smoke.mjs` | Bash is a thin launcher. Missing Node blocks Write/Edit unless `NOVA_WRITE_GUARD_DISABLED=1` is explicitly set; that bypass is not release evidence. |
