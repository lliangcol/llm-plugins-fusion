...
| Active write guard | Node.js 22+ and Bash 3.2+ | `node scripts/validate-runtime-smoke.mjs` | PreToolUse keeps a Bash fail-closed launcher because a missing direct Node command is non-blocking in the supported Claude runtime; post-use audit events use direct Node exec form. NotebookEdit fails closed. `NOVA_WRITE_GUARD_DISABLED=1` is not release evidence. |
...
| Windows maintenance smoke | Windows runner with Node.js 22, PowerShell, and Git Bash | CI `windows-node-smoke` and `windows-bash-smoke` jobs | Node/PowerShell checks run separately from Bash hook syntax and Codex runtime smoke so skipped local Bash checks have CI evidence. |
