...
| Roadmap and migration | Maintainers and authors | vNext, v1.2.0, and v2.0.0 boundaries, breaking-change expectations | Ready from roadmap and release decision doc |
...
the breaking release boundary. The `v1.2.0` row is the non-breaking registry and
scaffold lane from the roadmap and must be renumbered or skipped if the next
actual release is major.
...
| v1.2.0 | Registry-backed marketplace preparation. Make multi-entry registry data practical and keep catalog fields generated from source files. | Registry generation hardening, scaffold documentation, example/demo documentation, optional generated Markdown catalog if it has no new deploy dependency. | Monorepo restructure, breaking plugin path changes, mandatory frontend stack, release signing or SBOM pipeline requirements. |
| v2.0.0 | Breaking marketplace structure. Move from single primary plugin layout to explicit multi-plugin repository layout with migration guidance. | `plugins/*` layout, `nova-plugin` migration plan, multi-plugin catalog, public portal implementation decision if separately approved. | Treating v2.0.0 path changes as silent internals; shipping without migration docs and changelog `BREAKING` notes. |
...
- This document exists under `docs/marketplace/` and is linked from the project
  navigation.
- The `vNext`, `v1.2.0`, and `v2.0.0` boundaries are explicitly separated.
