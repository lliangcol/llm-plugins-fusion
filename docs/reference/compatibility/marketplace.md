<!-- migrated-from: docs/marketplace/compatibility-matrix.md -->
...
| Repository validation | Node.js 22+ and lockfile-pinned development dependencies installed with `npm ci --ignore-scripts` | `node scripts/validate-all.mjs`, including `node scripts/validate-runtime-smoke.mjs` and `node scripts/scan-distribution-risk.mjs` | The distributed `nova-plugin` archive has no Node runtime dependencies; maintainer validation still uses the locked Ajv and TypeScript development toolchain. |
...
| Active write guard | Node.js 22+ and Bash 3.2+ | `node scripts/validate-runtime-smoke.mjs` | Every active Node hook uses an exec-form privileged Bash launcher that rejects inherited startup/preload controls and project Node PATH shadows; missing trusted Node fails closed with exit 2. Startup project/local settings remain a host prerequisite, while ConfigChange blocks their mid-session activation. Existing targets must be regular single-link files. Hooks are not an atomic filesystem sandbox and PostToolUse cannot roll back a completed write. NotebookEdit and `NOVA_WRITE_GUARD_DISABLED=1` fail closed. |
| Hook syntax checks | Bash 3.2+ on PATH | `bash -n nova-plugin/hooks/scripts/pre-write-check.sh`, `bash -n nova-plugin/hooks/scripts/pre-bash-check.sh`, `bash -n nova-plugin/hooks/scripts/trusted-node-hook.sh`, and `bash -n nova-plugin/hooks/scripts/post-audit-log.sh` | Windows without Bash may warning-skip local syntax checks; CI/Linux and CI/Windows Bash smoke must run them. |
...
| Optional enhanced tools | Installed plugins or tools named by capability packs | `docs/reference/architecture/agent-routing.md` and `nova-plugin/packs/` | Enhanced mode is optional; fallback mode must remain documented. |
