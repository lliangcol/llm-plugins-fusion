...
| Install the stable Claude Code plugin | [getting-started.md](getting-started.md) | `nova-plugin` installed from exact tag `v4.0.0`, then `/nova-plugin:route` responds. |
| Use the contracts from another assistant | [consumers/README.md](consumers/README.md) | A consumer-owned profile that does not assume Claude slash-command behavior. |
| Evaluate compatibility evidence | [compatibility/README.md](compatibility/README.md) | Declared support, known-good lanes, current evidence, and historical evidence are distinguished. |
...
| Maintain or release the repository | [maintainers/quickstart.md](maintainers/quickstart.md) and [maintainers/release-runbook.md](maintainers/release-runbook.md) | The relevant focused checks and release gates are identified before mutation. |
| Find a maintenance command | [maintainers/task-catalog.md](maintainers/task-catalog.md) | The owning script, npm shortcut, and CI job are visible from one generated catalog. |
