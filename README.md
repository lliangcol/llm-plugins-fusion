...
只读 `nova-plugin` workflow 只需要 Claude Code 插件。启用 Write/Edit guard 的写入流程需要 Node.js 20+ 与 Bash 3.2+；维护仓库或运行本地校验同样需要 Node.js 20+；Codex 闭环命令还需要本机 Codex CLI。

稳定安装入口以正式 release tag 为准。当前稳定推广基线是 `v2.4.1`；
...
稳定推广对象是正式 release tag，例如 `v2.4.1`。`main` 可能包含 `CHANGELOG.md` `Unreleased` 下的后续文档或优化工作，不能替代 release tag 作为安装、推广或发布证据。
