...
[![Version](https://img.shields.io/badge/version-3.0.1-blue.svg)](https://github.com/lliangcol/llm-plugins-fusion/releases/tag/v3.0.1)
...
稳定安装入口以正式 release tag 为准。当前稳定推广基线是 `v3.0.1`；
`main` 可能包含 `CHANGELOG.md` `Unreleased` 下的后续优化，不能替代 exact
release tag 作为稳定发布证据。

```text
/plugin marketplace add lliangcol/llm-plugins-fusion@v3.0.1
...
稳定推广对象是正式 release tag，例如 `v3.0.1`。`main` 可能包含 `CHANGELOG.md` `Unreleased` 下的后续文档或优化工作，不能替代 release tag 作为安装、推广或发布证据。
