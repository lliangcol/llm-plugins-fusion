...
Before answering, use Read to load both `${CLAUDE_PLUGIN_ROOT}/runtime/contracts/route.json` as the machine-readable policy summary and `${CLAUDE_PLUGIN_ROOT}/skills/nova-route/SKILL.md` as the authoritative behavioral contract, then execute the workflow directly. If either contract cannot be loaded or they conflict, fail closed and report contract drift.
