...
Before answering, use Read to load `${CLAUDE_PLUGIN_ROOT}/runtime/contracts/implement-plan.json` as the compiled runtime contract, then apply it directly. The full compatibility skill is a maintainer reference and is not required for ordinary direct execution.
