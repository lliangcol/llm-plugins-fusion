...
| 场景类别     | 具体场景       | 推荐命令                                               | 跳转                           |
| ------------ | -------------- | ------------------------------------------------------ | ------------------------------ |
| **路由选择** | 不确定下一步命令 | `/nova-plugin:route`                                             | [文档](../commands/explore/route.md) |
| **需求分析** | 新功能需求理解 | `/nova-plugin:senior-explore`                                      | [示例](#场景-新功能需求分析)   |
| **需求分析** | 快速对齐认知   | ⭐`/nova-plugin:explore` 或 `/nova-plugin:explore-lite`                        | [示例](#场景-快速认知对齐)     |
| **需求分析** | 需求文档评审   | ⭐`/nova-plugin:explore PERSPECTIVE=reviewer` 或 `/nova-plugin:explore-review` | [示例](#场景-需求文档评审)     |
| **故障排查** | 生产问题调查   | `/nova-plugin:senior-explore`                                      | [示例](#场景-生产问题深度排查) |
| **故障排查** | 快速问题定位   | ⭐`/nova-plugin:explore` 或 `/nova-plugin:explore-lite`                        | [示例](#场景-快速问题定位)     |
| **方案设计** | 轻量任务计划   | `/nova-plugin:plan-lite`                                           | [示例](#场景-小型任务规划)     |
| **方案设计** | 正式设计文档   | `/nova-plugin:produce-plan`                                        | [示例](#场景-正式设计文档)     |
| **方案设计** | Java后端设计   | `/nova-plugin:backend-plan`                                        | [示例](#场景-java后端设计)     |
| **方案评审** | 计划文档评审   | `/nova-plugin:plan-review`                                         | [示例](#场景-计划文档评审)     |
| **代码评审** | 日常PR评审     | ⭐`/nova-plugin:review LEVEL=lite` 或 `/nova-plugin:review-lite`                | [示例](#场景-日常pr评审)       |
| **代码评审** | 核心逻辑评审   | ⭐`/nova-plugin:review` 或 `/nova-plugin:review-only`                          | [示例](#场景-核心逻辑评审)     |
| **代码评审** | 高风险代码审计 | ⭐`/nova-plugin:review LEVEL=strict` 或 `/nova-plugin:review-strict`           | [示例](#场景-高风险代码审计)   |
| **闭环修复** | 当前分支闭环修复 | `/nova-plugin:codex-review-fix`                                 | [示例](#工作流-c2-codex-闭环修复) |
| **闭环修复** | 只生成 Codex review | `/nova-plugin:codex-review-only`                             | [示例](#工作流-c2-codex-闭环修复) |
| **闭环修复** | 只做 Codex verify | `/nova-plugin:codex-verify-only`                             | [示例](#工作流-c2-codex-闭环修复) |
| **代码实现** | 严格按计划执行 | `/nova-plugin:implement-plan`                                      | [示例](#场景-按计划实现)       |
| **代码实现** | 标准开发任务   | `/nova-plugin:implement-standard`                                  | [示例](#场景-标准开发任务)     |
| **代码实现** | 快速小修复     | `/nova-plugin:implement-lite`                                      | [示例](#场景-快速修复)         |
| **工作收尾** | 完整交付物     | `/nova-plugin:finalize-work`                                       | [示例](#场景-完整工作交付)     |
| **工作收尾** | 快速总结       | `/nova-plugin:finalize-lite`                                       | [示例](#场景-快速工作总结)     |

`/nova-plugin:route` 是只读第一阶段路由器：先判断请求属于 explore、plan、review、
implement、finalize 或 Codex loop，再推荐一个下一步命令、skill、core
agent、capability packs、必需输入、验证建议和 fallback。它不写计划、不改代码、
不运行验证。
...
下图展示 Explore -> Plan -> Review -> Implement -> Finalize 主流程。3 个 Codex 闭环命令见下方对比表。
...
| ---- | --------------------- | -------- | ---------- | ---------- | ------------------------------------------------ |
| 路由 | `/nova-plugin:route`              | 🟡 中    | 路由建议   | ❌ 禁止    | 只读第一阶段路由：命令 / skill / agent / pack / 验证建议 |
| 探索 | `/nova-plugin:senior-explore`     | 🔴 强    | 分析报告   | ❌ 禁止    | 等价于 `/nova-plugin:explore DEPTH=deep`                     |
| 探索 | ⭐`/nova-plugin:explore`          | 🟡 中    | 视角化分析 | ❌ 禁止    | **统一命令，推荐**                               |
| 探索 | `/nova-plugin:explore-lite`       | 🟡 中    | 简要分析   | ❌ 禁止    | 等价于 `/nova-plugin:explore PERSPECTIVE=observer DEPTH=lite` |
| 探索 | `/nova-plugin:explore-review`     | 🟡 中    | 评审问题   | ❌ 禁止    | 等价于 `/nova-plugin:explore PERSPECTIVE=reviewer`           |
| 规划 | `/nova-plugin:plan-lite`          | 🟡 中    | 轻量计划   | ❌ 禁止    | 等价于 `/nova-plugin:produce-plan PLAN_PROFILE=lite`         |
| 规划 | `/nova-plugin:produce-plan`       | 🔴 强    | 正式文档   | ❌ 禁止    | 支持 profile 参数                                |
| 规划 | `/nova-plugin:backend-plan`       | 🔴 强    | 设计文档   | ❌ 禁止    | 等价于 `/nova-plugin:produce-plan PLAN_PROFILE=java-backend` |
| 规划 | `/nova-plugin:plan-review`        | 🟡 中    | 评审意见   | ❌ 禁止    | 等价于 `/nova-plugin:review REVIEW_PROFILE=plan`             |
| 评审 | `/nova-plugin:review-lite`        | 🟢 弱    | 反馈列表   | ❌ 禁止    | 等价于 `/nova-plugin:review LEVEL=lite`                     |
| 评审 | ⭐`/nova-plugin:review`           | 🟡-🔴    | 分级问题   | ❌ 禁止    | **统一命令，推荐**                               |
| 评审 | `/nova-plugin:review-only`        | 🟡 中    | 分级问题   | ❌ 禁止    | 等价于 `/nova-plugin:review LEVEL=standard MODE=findings-only` |
| 评审 | `/nova-plugin:review-strict`      | 🔴 强    | 全面审计   | ❌ 禁止    | 等价于 `/nova-plugin:review LEVEL=strict`                    |
| 评审 | `/nova-plugin:codex-review-only`  | 🟡 中    | review 工件 | ❌ 禁止    | 等价于 `/nova-plugin:review REVIEW_PROFILE=codex-review-only` |
| 评审 | `/nova-plugin:codex-verify-only`  | 🟡 中    | verify 工件 | ❌ 禁止    | 等价于 `/nova-plugin:review REVIEW_PROFILE=codex-verify-only`；需要 `REVIEW_FILE` |
| 实现 | `/nova-plugin:implement-plan`     | 🔴 强    | 代码变更   | ✅ 必须    | -                                                |
| 实现 | `/nova-plugin:implement-standard` | 🟡 中    | 代码变更   | ✅ 必须    | 等价于 `/nova-plugin:implement-plan EXECUTION_PROFILE=standard` |
| 实现 | `/nova-plugin:implement-lite`     | 🟢 弱    | 代码变更   | ✅ 必须    | 等价于 `/nova-plugin:implement-plan EXECUTION_PROFILE=lite`  |
| 实现 | `/nova-plugin:codex-review-fix`   | 🔴 强    | review/fix/verify 闭环 | ✅ 必须 | 等价于 `/nova-plugin:implement-plan EXECUTION_PROFILE=codex-review-fix` |
| 收尾 | `/nova-plugin:finalize-work`      | 🔴 强    | 交付文档   | ❌ 禁止    | -                                                |
| 收尾 | `/nova-plugin:finalize-lite`      | 🟢 弱    | 简要总结   | ❌ 禁止    | 等价于 `/nova-plugin:finalize-work DEPTH=lite`                |

**命令数量**: 21 个（18 个工作流命令 + 3 个 Codex 闭环命令）
**推荐路径**: 优先使用 ⭐ 标记的统一命令
...
### `/nova-plugin:senior-explore` — 深度探索分析
...
| `CONTEXT`     | ✅ 是   | 上下文材料   | 需求文档、代码路径、日志等            |
| `CONSTRAINTS` | ⚪ 可选 | 分析边界约束 | `Only analyze current implementation` |
| `DEPTH`       | ⚪ 可选 | 固定深度     | `deep`（省略时同样为 `deep`）         |
...
### `/nova-plugin:explore` — 统一探索命令 ⭐ 推荐

#### 📌 命令定位

```
角色：资深工程师
目的：通过参数化视角完成快速理解与认知对齐
优势：统一命令，减少选择成本
```
...
##### 场景: 快速理解新需求（observer 视角）

```text
/nova-plugin:explore PERSPECTIVE=observer
需求：用户希望在订单详情页添加"一键复购"功能
- 现有接口: POST /api/orders (创建订单)
- 业务规则: 复购需跳过已下架商品
...
| PERSPECTIVE | 等价命令          | 说明                     |
| ----------- | ----------------- | ------------------------ |
| `observer`  | `/nova-plugin:explore-lite`   | 观察者视角，快速认知对齐 |
| `reviewer`  | `/nova-plugin:explore-review` | 评审者视角，质询式探索   |

**推荐使用**：优先使用 `/nova-plugin:explore`，原有命令保留向后兼容。

---

### `/nova-plugin:explore-lite` — 轻量快速探索
...
### `/nova-plugin:explore-review` — 评审视角探索
...
背景：
- /nova-plugin:senior-explore 发现回调可能重复处理
- 原因：Redis 锁释放时机问题
...
### `/nova-plugin:produce-plan` — 正式设计文档
...
| `ANALYSIS_INPUTS`  | 🔶 强烈建议 | 引用的分析产物（如 /nova-plugin:senior-explore 的结果） |
...
### `/nova-plugin:backend-plan` — Java/Spring 后端设计
...
请用 /nova-plugin:plan-review 的结构评审这个方案
```
...
### `/nova-plugin:review` — 统一代码评审命令 ⭐ 推荐
...
public void settleAccount(Long accountId, BigDecimal amount) {
...
```text
/nova-plugin:review LEVEL=standard
新设计的 REST API，请评审:
...
| LEVEL      | 等价命令         | 说明                     |
| ---------- | ---------------- | ------------------------ |
| `lite`     | `/nova-plugin:review-lite`   | 轻量快速评审             |
| `standard` + `MODE=findings-only` | `/nova-plugin:review-only` | 标准 findings-only 评审 |
| `strict`   | `/nova-plugin:review-strict` | 严格代码审计（9 项维度） |

**推荐使用**：优先使用 `/nova-plugin:review`，原有命令保留向后兼容。

`/nova-plugin:review-lite` 是 `/nova-plugin:review LEVEL=lite` 的兼容快捷入口。

---

### `/nova-plugin:review-lite` — 轻量代码评审
...
### `/nova-plugin:review-only` — 常规严格评审
...
### `/nova-plugin:review-strict` — 高风险穷尽式评审
...
### `/nova-plugin:implement-plan` — 严格按计划实现
...
### `/nova-plugin:implement-standard` — 标准受控实现
...
### `/nova-plugin:implement-lite` — 快速实现
...
### `/nova-plugin:finalize-work` — 完整工作交付
...
### `/nova-plugin:finalize-lite` — 极简收尾
...
```
┌─────────────────────────────────────────────────────────┐
│  1️⃣ /nova-plugin:senior-explore
│     → 明确已知/未知/风险（不提方案）
│     → 输出分析报告
├─────────────────────────────────────────────────────────┤
│  2️⃣ /nova-plugin:plan-lite
│     → 快速对齐目标、非目标、方法
│     → 轻量计划
├─────────────────────────────────────────────────────────┤
│  3️⃣ /nova-plugin:produce-plan （如需正式评审
│     → 写入正式设计文档
├─────────────────────────────────────────────────────────┤
│  4️⃣ /nova-plugin:plan-review
│     → 评审问题提前暴露
├─────────────────────────────────────────────────────────┤
│  5️⃣ /nova-plugin:implement-plan 或 /nova-plugin:implement-standard
│     → 执行实现
├─────────────────────────────────────────────────────────┤
│  6️⃣ /nova-plugin:review-only
│     → 代码自查
├─────────────────────────────────────────────────────────┤
│  7️⃣ /nova-plugin:finalize-work
│     → 生成 commit + PR
└─────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────┐
│  1️⃣ /nova-plugin:senior-explore (deep)
│     → 还原事实与假设
│     → 导出分析报告
├─────────────────────────────────────────────────────────┤
│  2️⃣ /nova-plugin:plan-lite
│     → 快速确定修复方案
├─────────────────────────────────────────────────────────┤
│  3️⃣ /nova-plugin:implement-standard 或 /nova-plugin:implement-lite
│     → 快速实现修复
├─────────────────────────────────────────────────────────┤
│  4️⃣ /nova-plugin:review-strict
│     → 修复代码严格审查
├─────────────────────────────────────────────────────────┤
│  5️⃣ /nova-plugin:finalize-work
│     → 严格收尾
└─────────────────────────────────────────────────────────┘
```
...
小改动 → /nova-plugin:review LEVEL=lite
核心链路 → /nova-plugin:review LEVEL=standard
高风险/并发/金融 → /nova-plugin:review LEVEL=strict
```
...
```
1. /nova-plugin:codex-review-only 或直接 /nova-plugin:codex-review-fix
2. Claude Code 修复高置信问题
3. 运行仓库 checks
4. /nova-plugin:codex-verify-only
...
│     → 分析需求和现有系统
├─────────────────────────────────────────────────────────┤
│  2️⃣ /nova-plugin:backend-plan
│     → 产出完整后端设计文档
│     → 包含事务、幂等、可观测性设计
├─────────────────────────────────────────────────────────┤
│  3️⃣ /nova-plugin:plan-review
│     → 设计文档评审
├─────────────────────────────────────────────────────────┤
│  4️⃣ /nova-plugin:implement-plan
│     → 严格按设计实现
├─────────────────────────────────────────────────────────┤
│  5️⃣ /nova-plugin:review-strict
│     → 高标准代码审查
├─────────────────────────────────────────────────────────┤
│  6️⃣ /nova-plugin:finalize-work
│     → 交付物
└─────────────────────────────────────────────────────────┘
```
...
| 命令              | 一句话描述         | 输出结构                             |
| ----------------- | ------------------ | ------------------------------------ |
| `/nova-plugin:senior-explore` | 深度分析，暴露风险 | Findings / Questions / Risks         |
| `/nova-plugin:explore-lite`   | 快速对齐理解       | Observations / Uncertainties / Risks |
| `/nova-plugin:explore-review` | Reviewer 视角质询  | Clear / Questions / Risk signals     |

### Plan 命令速查

| 命令            | 一句话描述    | 输出位置 |
| --------------- | ------------- | -------- |
| `/nova-plugin:plan-lite`    | 轻量执行计划  | 聊天输出 |
| `/nova-plugin:produce-plan` | 正式设计文档  | 写入文件 |
| `/nova-plugin:backend-plan` | Java 后端设计 | 写入文件 |
| `/nova-plugin:plan-review`  | 计划质量评审  | 聊天输出 |

### Review 命令速查

| 命令             | 适用场景   | 深度  |
| ---------------- | ---------- | ----- |
| `/nova-plugin:review LEVEL=lite` / `/nova-plugin:review-lite` | 日常 PR    | 🟢 轻 |
| `/nova-plugin:review LEVEL=standard MODE=findings-only` / `/nova-plugin:review-only` | 核心链路 findings-only 评审 | 🟡 中 |
| `/nova-plugin:review LEVEL=strict` / `/nova-plugin:review-strict` | 高风险审计 | 🔴 深 |
| `/nova-plugin:codex-review-only` | 分支 review 工件化 | 🟡 中 |
| `/nova-plugin:codex-verify-only` | 定向复验 | 🟡 中 |

### Implement 命令速查

| 命令                  | 适用场景     | 约束强度 |
| --------------------- | ------------ | -------- |
| `/nova-plugin:implement-plan`     | 有批准的计划 | 🔴 强    |
| `/nova-plugin:implement-standard` | 明确步骤     | 🟡 中    |
| `/nova-plugin:implement-lite`     | 快速小任务   | 🟢 弱    |
| `/nova-plugin:codex-review-fix`   | review/fix/verify 闭环 | 🔴 强 |

### Finalize 命令速查

| 命令             | 适用场景 | 输出内容    |
| ---------------- | -------- | ----------- |
| `/nova-plugin:finalize-work` | 完整交付 | commit + PR |
| `/nova-plugin:finalize-lite` | 快速总结 | 三要素      |
...
| 错误用法                               | 问题                   | 正确用法                       |
| -------------------------------------- | ---------------------- | ------------------------------ |
| 用 `/nova-plugin:senior-explore` 后直接给方案      | 违反"只分析不设计"原则 | 分析完用 `/nova-plugin:plan-lite`          |
| `/nova-plugin:implement-plan` 不提供 PLAN_APPROVED | 会被阻塞               | 必须显式 `PLAN_APPROVED: true` |
| 用 `/nova-plugin:review-lite` 审核支付代码         | 深度不够               | 应使用 `/nova-plugin:review-strict`        |
| `/nova-plugin:finalize-work` 时还在改代码          | 违反"冻结现状"原则     | 先完成变更再收尾               |
