...
Status: generated from `workflow-specs/workflows.json` and `workflow-specs/adapters/codex.json`. Do not edit this file by hand.
...
| --- | --- | --- |
| Intent explicitly requires the external Codex review, fix, and verify closure loop. | codex-review-fix | Route to the complete external closure workflow. |
| Intent explicitly requires external Codex review only with no fixes. | codex-review-only | Route to external review only. |
| Intent explicitly requires verification of an existing external review artifact. | codex-verify-only | Route to external verification only. |
| Intent requires a deep or systematic analysis artifact without solutioning. | senior-explore | Route to senior exploration. |
| Intent requires reviewer-perspective exploration questions without solutions. | explore-review | Route to reviewer exploration. |
| Intent requires factual understanding, unknowns, or risk without solutions. | explore | Route to the exploration hub. |
| Intent explicitly requires a Java or Spring twelve-section backend design. | backend-plan | Route to the specialized backend plan. |
| Intent requires review of an existing plan without rewriting it. | plan-review | Route to plan review. |
| Intent requires a lightweight plan with no artifact file. | plan-lite | Route to lightweight planning. |
| Intent requires a formal review-ready plan artifact at an explicit path. | produce-plan | Route to plan production. |
| Intent requires a strict production-critical or security audit. | review-strict | Route to strict review. |
| Intent requires a fast review of a small patch with only obvious findings. | review-lite | Route to lightweight review. |
| Intent requires standard severity-grouped read-only review with no implementation. | review-only | Route to review only. |
| Intent requires review but no narrower review contract is explicit. | review | Route to the review hub. |
| Intent requires execution of an explicitly approved plan file. | implement-plan | Route to approved-plan implementation. |
| Intent requires a small bounded fix with focused validation. | implement-lite | Route to lightweight implementation. |
| Intent requires execution of explicit confirmed steps. | implement-standard | Route to standard implementation. |
