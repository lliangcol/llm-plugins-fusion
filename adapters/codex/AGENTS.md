...
| --- | --- | --- | --- | --- |
| `{"op":"semantic-condition","condition":"Intent explicitly requires the external Codex review, fix, and verify closure loop."}` | implement-plan | `{"EXECUTION_PROFILE":"codex-review-fix"}` | codex-review-fix | Route to the complete external closure workflow. |
| `{"op":"semantic-condition","condition":"Intent explicitly requires external Codex review only with no fixes."}` | review | `{"REVIEW_PROFILE":"codex-review-only"}` | codex-review-only | Route to external review only. |
| `{"op":"semantic-condition","condition":"Intent explicitly requires verification of an existing external review artifact."}` | review | `{"REVIEW_PROFILE":"codex-verify-only"}` | codex-verify-only | Route to external verification only. |
| `{"op":"semantic-condition","condition":"Intent requires a deep or systematic analysis artifact without solutioning."}` | explore | `{"DEPTH":"deep"}` | senior-explore | Route to senior exploration. |
| `{"op":"semantic-condition","condition":"Intent requires reviewer-perspective exploration questions without solutions."}` | explore | `{"PERSPECTIVE":"reviewer"}` | explore-review | Route to reviewer exploration. |
| `{"op":"semantic-condition","condition":"Intent requires factual understanding, unknowns, or risk without solutions."}` | explore | `{}` | explore | Route to the exploration hub. |
| `{"op":"semantic-condition","condition":"Intent explicitly requires a Java or Spring twelve-section backend design."}` | produce-plan | `{"PLAN_PROFILE":"java-backend"}` | backend-plan | Route to the specialized backend plan. |
| `{"op":"semantic-condition","condition":"Intent requires review of an existing plan without rewriting it."}` | review | `{"REVIEW_PROFILE":"plan"}` | plan-review | Route to plan review. |
| `{"op":"semantic-condition","condition":"Intent requires a lightweight plan with no artifact file."}` | produce-plan | `{"PLAN_PROFILE":"lite"}` | plan-lite | Route to lightweight planning. |
| `{"op":"semantic-condition","condition":"Intent requires a formal review-ready plan artifact at an explicit path."}` | produce-plan | `{}` | produce-plan | Route to plan production. |
| `{"op":"semantic-condition","condition":"Intent requires a strict production-critical or security audit."}` | review | `{"LEVEL":"strict"}` | review-strict | Route to strict review. |
| `{"op":"semantic-condition","condition":"Intent requires a fast review of a small patch with only obvious findings."}` | review | `{"LEVEL":"lite"}` | review-lite | Route to lightweight review. |
| `{"op":"semantic-condition","condition":"After external, plan-review, strict, and lite review conditions are excluded, intent requires a read-only or findings-only review and prohibits implementation or modification; explicit severity grouping is not required."}` | review | `{"LEVEL":"standard","MODE":"findings-only"}` | review-only | Route to canonical review in standard findings-only mode. |
| `{"op":"semantic-condition","condition":"Intent requires a general review, does not specify a read-only, findings-only, no-change, lite, strict, external, or plan-review boundary, and no narrower review contract matches."}` | review | `{}` | review | Route to the review hub. |
| `{"op":"semantic-condition","condition":"Intent requires execution of an explicitly approved plan file."}` | implement-plan | `{}` | implement-plan | Route to approved-plan implementation. |
| `{"op":"semantic-condition","condition":"Intent requires a small bounded fix with focused validation."}` | implement-plan | `{"EXECUTION_PROFILE":"lite"}` | implement-lite | Route to lightweight implementation. |
| `{"op":"semantic-condition","condition":"Intent requires execution of explicit confirmed steps."}` | implement-plan | `{"EXECUTION_PROFILE":"standard"}` | implement-standard | Route to standard implementation. |
| `{"op":"semantic-condition","condition":"Intent requires a short factual closure summary only."}` | finalize-work | `{"DEPTH":"lite"}` | finalize-lite | Route to lightweight finalization. |
| `{"op":"semantic-condition","condition":"Intent requires a complete handoff or pull-request-ready closeout."}` | finalize-work | `{}` | finalize-work | Route to full finalization. |
