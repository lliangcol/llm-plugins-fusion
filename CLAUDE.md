...
**LLM Plugins Fusion** 是一个面向 LLM 编程助手（如 Claude Code）的第三方插件市场。核心模块 `nova-plugin` 提供一套覆盖"探索→计划→实现→评审→收尾"全流程的命令体系，辅以 `nova-plugin-command-generator` React 工具 UI。
...
| Review | `review`, `review-lite`, `review-only`, `review-strict` | 代码质量审查 |
| Implement | `implement-plan`, `implement-standard`, `implement-lite` | 执行，严格程度递减 |
