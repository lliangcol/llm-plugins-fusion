...
function parseArgs(args, env) {
  const options = { repository: env.GITHUB_REPOSITORY, commit: env.GITHUB_SHA, candidateActor: env.GITHUB_ACTOR, token: env.GH_TOKEN ?? env.GITHUB_TOKEN, out: resolve(root, '.metrics/release-review/independent-review.json') };
...
export async function verifyIndependentReview({ args = process.argv.slice(2), env = process.env, fetchImpl = fetch, now = () => new Date() } = {}) {
  ...
  const evidence = { schemaVersion: 1, repository: options.repository, commit: options.commit, pullRequest: pull.number, pullRequestHead, ...result, checkedAt: now().toISOString() };
