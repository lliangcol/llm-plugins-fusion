...
import { buildReleaseCandidate, independentReviewProvesCandidate } from './lib/release-candidate.mjs';
...
export function parseCandidateArgs(args, env = process.env) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (arg === '--out-dir') options.outDir = resolveArtifactOutputPath(root, value(), 'release candidate output directory');
...
/** @param {{args?: string[], env?: NodeJS.ProcessEnv, now?: () => Date, correctionSource?: {document: {corrections: object[]}, sha256: string} | null}} [options] */
export function generateReleaseCandidate({ args = process.argv.slice(2), env = process.env, now, correctionSource = null } = {}) {
  const options = parseCandidateArgs(args, env);
  const repoBoundary = createPhysicalReadBoundary(root, 'release candidate repository root');
  const bundleBoundary = createPhysicalReadBoundary(options.bundleRoot, 'release candidate bundle root');
  const controlBoundary = createPhysicalReadBoundary(dirname(options.bundleRoot), 'release candidate handoff root');
  ...
  const review = reviewPath
    ? JSON.parse(readPhysicalFile(bundleBoundary, reviewPath, 'independent review evidence').buffer.toString('utf8'))
    : null;
  const reviewerPolicy = validateReleaseReviewerPolicy(JSON.parse(
    readPhysicalFile(
      repoBoundary,
      resolve(root, 'governance/release-reviewers.json'),
      'release reviewer policy',
    ).buffer.toString('utf8'),
  ));
  const independentReview = {
    passed: independentReviewProvesCandidate(review, { commit: options.commit }, reviewerPolicy),
  };
  const releasePolicy = assertReleaseReady(evaluateReleaseCorrections({
    ...
    corrections: resolvedCorrectionSource.document.corrections, correctionsSha256: resolvedCorrectionSource.sha256, independentReview,
  }));
  const controlManifestFile = readPhysicalFile(controlBoundary, options.controlBundleManifest, 'release control manifest');
  const controlManifest = JSON.parse(controlManifestFile.buffer.toString('utf8'));
  const controlBundlePath = resolve(dirname(options.controlBundleManifest), 'release-control-bundle.tar.gz');
  const controlBundleFile = readPhysicalFile(controlBoundary, controlBundlePath, 'release control bundle');
  verifyControlBundle({ bundlePath: controlBundlePath, manifest: controlManifest, archive: controlBundleFile.buffer });
  ...
  const outputPlan = prepareArtifactOutputPlan(root, [
    { key: 'core', path: paths.core, label: 'release candidate core output' },
    { key: 'intent', path: paths.intent, label: 'release promotion intent output' },
    { key: 'envelope', path: paths.envelope, label: 'release candidate envelope output' },
  ], {
    protectedPaths: [options.controlBundleManifest, ...options.evidencePaths],
    protectedRoots: [options.artifactDir],
  });
  writeArtifactOutput(outputPlan, 'core', `${JSON.stringify(core, null, 2)}\n`);
  writeArtifactOutput(outputPlan, 'intent', `${JSON.stringify(intent, null, 2)}\n`);
  writeArtifactOutput(outputPlan, 'envelope', `${JSON.stringify(envelope, null, 2)}\n`);
