...
export function buildInventory() {
  ...
  return {
    ...
    generators: scripts.filter((name) => name.startsWith('generate-') || name === 'sync-doc-facts.mjs').map((name) => ({ path: `scripts/${name}`, owner: 'maintainer control plane', outputScope: 'generated projection' })),
...
export function main(args = process.argv.slice(2)) {
  try {
    if (args.length > 1 || (args.length === 1 && args[0] !== '--write')) throw new Error('Usage: node scripts/generate-control-plane-inventory.mjs [--write]');
    const data = buildInventory();
    const json = `${JSON.stringify(data, null, 2)}\n`;
    const markdown = render(data);
    if (args[0] === '--write') { writeFileSync(jsonPath, json, 'utf8'); writeFileSync(markdownPath, markdown, 'utf8'); }
    else {
    ...
    console.log(`OK control-plane inventory (${data.validationTasks.length} validation tasks)`);
    return 0;
