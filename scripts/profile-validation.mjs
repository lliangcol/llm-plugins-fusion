...
export async function main({ args = process.argv.slice(2), runner = runProcess, mkdirFn = mkdir, env = process.env } = {}) {
  ...
  const result = await runner(benchmark ? 'benchmark validate-all' : 'profile validate-all', process.execPath, childArgs, {
    ...
    env: { ...withoutGithubCredentials(env), NOVA_VALIDATION_SCENARIO: benchmark ? 'fresh-process-full-uncached' : 'cpu-profile' },
  });
  ...
  if (benchmark) {
    ...
    const validation = await runner('validate observed performance', process.execPath, validateArgs, {
      cwd: root, capture: false, timeoutMs: 30_000, env,
    });
