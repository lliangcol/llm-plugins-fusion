...
function validateWorkflowContracts() {
  ...
  if (promotionSrc) {
    ...
    for (const [pattern, message] of [
      ...
      [/Rebuild candidate bytes for deterministic comparison only[\s\S]*GITHUB_REF_NAME:\s*\$\{\{\s*steps\.candidate\.outputs\.candidate_tag\s*\}\}[\s\S]*GITHUB_SHA:\s*\$\{\{\s*steps\.release\.outputs\.commit\s*\}\}[\s\S]*npm run release:artifacts[\s\S]*cmp /, 'promotion must rebuild only for byte comparison against candidate artifacts and bind candidate provenance to the verified release commit'],
