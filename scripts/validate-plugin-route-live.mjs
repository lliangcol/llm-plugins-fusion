...
export const routeSystemPrompt = `This is an automated contract validation of an explicitly invoked /nova-plugin:route command. Follow the loaded route command and nova-route skill. The final response MUST start with exactly "${routeOutputContract.heading}" and then contain exactly these seven Markdown bullet labels in this order: ${routeOutputContract.requiredFields.join(' ')} Use no preface, alternate heading level, table, renamed field, or closing text. For Command:, output only one installed /nova-plugin:<id>. For Skill:, Core agent:, and Capability packs:, output only installed canonical IDs separated by commas; do not add prose, labels, parentheses, or descriptions to those four inventory values. Fill every value using only the installed nova inventory.`;
