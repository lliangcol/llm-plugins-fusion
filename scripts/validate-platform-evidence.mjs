...
const sourcePath = resolve(root, 'governance/platform-evidence.json');
...
export function evaluatePlatformCapabilities({ windows, hasBash, hasPowerShell, containerRequested = false, containerReady = false }) {
  if (!windows && !hasBash) return { status: 'failed', reasonCode: 'REQUIRED_TOOL_UNAVAILABLE' };
  if (hasBash) return { status: 'passed', reasonCode: 'CHECK_PASSED' };
  if (containerRequested && !containerReady) return { status: 'skipped', reasonCode: 'CONTAINER_FALLBACK_UNAVAILABLE' };
...
export function main(args = process.argv.slice(2)) {
  try {
    ...
    if (container && (!source.containerFallback.enabled || !/^sha256:[a-f0-9]{64}$/u.test(source.containerFallback.imageDigest ?? ''))) throw new Error('container fallback unavailable: no approved digest-pinned image; use Git Bash, WSL, or CI Bash evidence');
    ...
    console.log(`OK platform evidence matrix (${source.tasks.length} tasks; container fallback disabled)`);
    return 0;
