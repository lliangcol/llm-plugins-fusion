...
test('candidate authorization is bound to the exact stable tag, RC tag, commit, and correction digest', () => {
  const correction = { ...base, status: 'authorized-for-new-candidate', targetRelease: identity };
...
test('promotion fails closed for unverified candidate or missing operational evidence', () => {
  const authorized = { ...base, status: 'authorized-for-new-candidate', targetRelease: identity };
...
test('drill mode can never advance beyond PROMOTION_READY', () => {
  const correction = { ...base, status: 'candidate-verified', targetRelease: identity };
...
test('correction evaluator rejects malformed inputs and unsafe source paths', () => {
  ...
  assert.throws(() => loadReleaseCorrections(process.cwd(), 'package.json'), /schemaVersion 2/u);
});

test('correction loading verifies evidence digests and rejects self-reference', () => {
  assert.equal(loadReleaseCorrections(process.cwd()).document.schemaVersion, 2);
  const directory = mkdtempSync(resolve(tmpdir(), 'correction-evidence-'));
  try {
    const path = resolve(directory, 'corrections.json');
    const document = { schemaVersion: 2, corrections: [{ id: 'X', auditTrail: [{ evidence: { path: 'corrections.json', sha256: '0'.repeat(64) } }] }] };
