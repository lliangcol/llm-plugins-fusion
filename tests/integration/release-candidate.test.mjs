...
test('candidate tag parsing binds an RC number to a stable base version', () => {
  assert.deepEqual(parseCandidateTag('v3.0.1-rc.2'), {
    tag: 'v3.0.1-rc.2', stableVersion: '3.0.1', number: 2,
  });
  for (const tag of ['v3.0.1', '3.0.1-rc.1', 'v3.0.1-beta.1', 'v3.0.1-rc.01']) {
...
test('candidate manifest binds source, evidence, and deterministic artifacts for promotion', async (t) => {
  ...
  assert.equal(fixture.manifest.candidate.stableVersion, '3.0.1');
  ...
  assert.equal(promoted.candidateTag, 'v3.0.1-rc.1');
...
test('candidate rehearsal rejects zero, multiple, and mismatched artifacts', async (t) => {
  ...
  await unlink(join(missing.artifactDir, 'nova-plugin-3.0.1.tar.gz'));
  assert.throws(() => resolveCandidateArtifacts(missing.artifactDir, '3.0.1'), /exactly one archive; found 0/);

  const multiple = await candidateFixture(t);
  await writeFile(join(multiple.artifactDir, 'nova-plugin-9.9.9.tar.gz'), 'duplicate');
  assert.throws(() => resolveCandidateArtifacts(multiple.artifactDir, '3.0.1'), /exactly one archive; found 2/);

  const changed = await candidateFixture(t);
  await writeFile(join(changed.artifactDir, 'nova-plugin-3.0.1.tar.gz'), 'changed');
...
test('promotion rejects candidate version, commit, and source drift', async (t) => {
  const fixture = await candidateFixture(t);
  assert.throws(() => verifyReleasePromotion({
    root, stableTag: 'v3.0.1', commit: 'b'.repeat(40), manifest: fixture.manifest, artifactDir: fixture.artifactDir,
  }), /same commit/);
  assert.throws(() => verifyReleasePromotion({
  ...
  assert.throws(() => verifyReleasePromotion({
    root, stableTag: 'v3.0.1', commit, manifest: drifted, artifactDir: fixture.artifactDir,
...
test('candidate and promotion CLI parsers preserve explicit evidence paths', async (t) => {
  ...
  assert.equal(JSON.parse(await readFile(out, 'utf8')).candidate.tag, 'v3.0.1-rc.3');
  assert.equal(parseCandidateArgs(['--tag', 'v3.0.1-rc.4', '--commit', commit], {}).tag, 'v3.0.1-rc.4');
  assert.equal(parsePromotionArgs(['--stable-tag', 'v3.0.1', '--commit', commit], {}).stableTag, 'v3.0.1');
});
